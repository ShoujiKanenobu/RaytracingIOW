...
inline float step(vec3 v, vec3 edge)
{
    if(v.x() < edge.x() && v.y() < edge.y() && v.z() < edge.z())
        return 1.0;
    return 0.0;
}
...
vec3 random_in_hemisphere (const vec3& normal)
{
    ...
    if(dot(in_unit_sphere, normal) > 0.0) // Is in same hemisphere as normal
